...
def run_simulation(
    ...
    df['solar_cost_per_km'] = df['solar_eff_Wh_per_km'] / 1000 * 0
    ...
    # --- SUPERCAPACITOR VS BATTERY EFFICIENCY PLOT (right only) ---
    fig_cap_vs_bat, ax_cap2 = plt.subplots(figsize=(7, 6))
    ...
    ax_cap2.plot(cycle_counts, supercap_efficiency, 'o-', linewidth=2.5, markersize=8, color='#A23B72', label='Supercapacitor')
    ax_cap2.plot(cycle_counts, battery_efficiency, 's-', linewidth=2.5, markersize=8, color='#2E86AB', label='Battery')
    ax_cap2.fill_between(cycle_counts, supercap_efficiency, battery_efficiency, alpha=0.2, color='#A23B72')
    ax_cap2.set_xlabel('Charge/Discharge Cycles', fontsize=11)
    ax_cap2.set_ylabel('Energy Efficiency (%)', fontsize=11)
    ax_cap2.set_title('Efficiency Over Lifecycle\nSupercapacitor vs Battery', fontsize=12, fontweight='bold')
    ax_cap2.set_xscale('log')
    ax_cap2.grid(True, linestyle='--', alpha=0.6)
    ax_cap2.legend(fontsize=10)
    ax_cap2.set_ylim(60, 100)
    ax_cap2.annotate('Supercapacitor maintains\n>95% efficiency', xy=(10000, 95.5), xytext=(5000, 85),
                     arrowprops=dict(arrowstyle='->', color='#A23B72', lw=1.5), fontsize=9, color='#A23B72', fontweight='bold')
    plt.tight_layout()

    # --- BATTERY DEGRADATION PLOT (left only, no total cost over years) ---
    fig_degradation, ax_deg1 = plt.subplots(figsize=(7, 6))
    ...
    ax_deg1.plot(years_degradation, solar_degradation, 'o-', linewidth=2.5, markersize=8, label='Solar TukTuk (Supercap-Assisted)')
    ax_deg1.plot(years_degradation, grid_degradation, 's-', linewidth=2.5, markersize=8, label='Grid TukTuk (Battery Only)')

    ax_deg1.fill_between(years_degradation, solar_degradation, grid_degradation, alpha=0.20, label='Capacity Advantage (Solar)')

    ax_deg1.axhline(80, color='red', linestyle='--', linewidth=1.4, label='80% Threshold (End of Life)')

    ax_deg1.set_title('Battery Capacity Degradation Over Time\nSolar (Supercap-Assisted) vs Grid', fontsize=12, fontweight='bold')
    ax_deg1.set_xlabel('Years of Operation', fontsize=11)
    ax_deg1.set_ylabel('Remaining Capacity (%)', fontsize=11)
    ...
    ax_deg1.annotate(
        f'Solar hits 80% at ~{solar_80_year:.1f} years',
        xy=(solar_80_year, 80),
        xytext=(solar_80_year + 1, 70),
        arrowprops=dict(arrowstyle='->', lw=1.4),
        fontsize=9, fontweight='bold'
    )

    ax_deg1.annotate(
        f'Grid hits 80% at ~{grid_80_year:.1f} years',
        xy=(grid_80_year, 80),
        xytext=(grid_80_year - 2, 90),
        arrowprops=dict(arrowstyle='->', lw=1.4),
        fontsize=9, fontweight='bold'
    )
